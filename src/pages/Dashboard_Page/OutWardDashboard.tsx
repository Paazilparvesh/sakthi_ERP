...
// // 🧭 Main Dashboard
// // -----------------------------
// const OutwardDashboard: React.FC = () => {
...
import React, { useEffect, useState, useCallback } from "react";
import { useToast } from "@/components/ui/use-toast";
import { ProductList } from "@/components/OutwardComponents/OutwardList";
import OutwardDetail from "@/components/OutwardComponents/OutwardDetail";
import { AddProcessForm } from "@/components/OutwardComponents/OutwardForm";
import { Product } from "@/types/role4.types";
...
const OutwardDashboard: React.FC = () => {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    const [productRes, qaRes] = await Promise.all([
      fetch(`${BASE_URL}/api/get_full_products/`),
      fetch(`${BASE_URL}/api/get_QA/`),
    ]);

    if (!productRes.ok || !qaRes.ok) throw new Error("Failed to fetch data");

    const productData = await productRes.json();
    const qaData = await qaRes.json();

    setProducts(productData.reverse());
    setqas(qaData.reverse());
  } catch (error) {
    console.error(error);
    toast({
      variant: "destructive",
